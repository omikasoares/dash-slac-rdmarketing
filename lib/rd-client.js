 ...
 * - Fila com espaçamento mínimo entre requisições (RD_MAX_REQUESTS_PER_MINUTE)
 *   pra evitar bater no rate limit em primeiro lugar, em vez de só reagir
 *   depois do 429 (o que é caro: 2s/4s/6s/8s de espera por tentativa).
 */
...
/**
 * Limitador de taxa global: espaça as requisições em pelo menos
 * `60000 / RD_MAX_REQUESTS_PER_MINUTE` ms entre si, independente de quantas
 * chamadas concorrentes estejam pendentes. Isso serializa só o *disparo* da
 * requisição (nao o processamento da resposta), entao ainda da pra ter varios
 * contatos em voo ao mesmo tempo sem estourar o limite do RD Station.
 */
const MAX_REQUESTS_PER_MINUTE = Number(process.env.RD_MAX_REQUESTS_PER_MINUTE) || 100;
...
function throttle() {
  ...
  // Garante que uma falha num elo da fila nao trave os proximos.
