...
// Migracao leve para bancos criados antes dessas colunas existirem.
const NEW_COLUMNS = [
  ['public_url', 'TEXT'],
  ['owner_email', 'TEXT'],
  ['total_conversions', 'INTEGER'],
...
// ainda nao tem — permite join rapido com a planilha de CRM sem parsear
...
// Bootstrap a partir da exportacao CSV do RD Station: popula todos os campos
// que o export traz de graca, mas NAO marca synced_last_conversion_date —
// assim o sync em segundo plano ainda busca o historico detalhado de
// conversao (com atribuicao de trafego por conversao) na proxima rodada,
// como se fosse um contato novo.
const upsertFromCsvStmt = db.prepare(`
  INSERT INTO contacts (
    ...
    @total_conversions, @first_conversion_date, @first_conversion_origin,
    @last_opportunity_date, @last_sale_date, @last_sale_value, @events_summary_raw,
    @produtos_comprados, 'csv_import', @id_crm
  )
  ON CONFLICT(uuid) DO UPDATE SET
    name = excluded.name,
    email = excluded.email,
